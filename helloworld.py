...
class MainPage(webapp.RequestHandler):
	...
	<body background="static/dhs-list-background4.jpg>
		...
		<h3> Instructions</h3>
		<h4> 
		1.)To create a new column on the class list, scroll down to the bottom of the page. 
			Enter a name for yout new column and click create. 
			Tick on the boxes and click update after selection.
			</br>
		2.) Only the creator of the column could edit the column.
			</br>
		3.) If an error message appears when creating a new column, please type in a new column name. ( make sure that it is not repeated, do not contain spaces. Some words such as class and try are not allowed due to the program of DHS LIST. We apologise for any inconvenice caused)
			</br>
		</h4>
		</div>
		<div>
